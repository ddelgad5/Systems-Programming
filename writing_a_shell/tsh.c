...
int builtin_cmd(char **argv); //  Recognizes and interprets built-in commands: quit, fg, bg, and jobs [25 lines]
...
void sigchld_handler(int sig);  //  Catches the SIGCHILD signals [15 lines]
void sigtstp_handler(int sig);  //  Catches the SIGINT (ctrl-c) signals [15 lines]
void sigint_handler(int sig); //  Catches the SIGSTP (ctrl-z) signals [15 lines]

/* Here are helper routines that we've provided for you */
int parseline(const char *cmdline, char **argv);  //  Parses command line into an array
void sigquit_handler(int sig);  // Catches the SIGQUIT (ctrl-\) signals
...
int main(int argc, char **argv)
{
  ...
  while (1) {
    ...
    eval(cmdline);
    fflush(stdout); // cleans out terminal line
    fflush(stdout); // cleans out terminal line
  }
